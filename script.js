...
let footerField = document.getElementsByTagName('footer')[0];
...
let hamburgerMenuField = document.getElementsByClassName('navbar-toggler')[0];
let headerField = document.getElementById('navbarHeader');

let onHamburgerMenuClick = function() {
	headerField.classList.toggle("collapse");
};
...
// Indice : Commence par bien pointer sur la première card en entier (stocke-la dans une variable 
// 	puis affiche-la en console pour confirmer que tu l'as bien pointé).
...
let firstCardEditButton = document.getElementsByClassName('btn-outline-secondary')[0];

let onFirstEditButtonClick = function() {
	firstCardText.style.color = "red";
};

firstCardEditButton.addEventListener("click", onFirstEditButtonClick);
...
let secondCardEditButton = document.getElementsByClassName('btn-outline-secondary')[1];

let onSecondEditButtonClick = function() {
...
let bootstrapCssLink = document.querySelectorAll('head>link')[0].href;

let navbarField = document.getElementsByClassName('navbar-dark')[0];

let onNavbarClick = function() {
	if(document.querySelectorAll('head>link')[0].href === bootstrapCssLink) {
	document.querySelectorAll('head>link')[0].href = '';
	}else{
		document.querySelectorAll('head>link')[0].href = bootstrapCssLink;
	};
};

headerField.addEventListener("dblclick", onNavbarClick);
navbarField.addEventListener("dblclick", onNavbarClick);
...
// et sur la fonctionnalité 3 (toggle sur le style appliqué à l'image : tantôt on lui rajoute "width: 20 %;", tantôt on lui enlève).
...
// => clique ici pour accéder à une ressource qui parle de ça

let cards = document.getElementsByClassName("col-md-4");

for (i=0; i<cards.length; i++) {
	let viewButton = cards[i].getElementsByClassName('btn-success')[0];
	let cardImage = cards[i].getElementsByClassName('card-img-top')[0];
	let cardText = cards[i].getElementsByClassName('card-text')[0];



	let reduceCardElement = function(){
...
viewButton.addEventListener("mouseover", reduceCardElement);
...
// une page s'ouvre ! 
// Il faut que tu bloques ce comportement par défaut (via l'objet event - cf le cours).
...
let leftButtonField = document.getElementsByClassName("my-2")[0];
function firstCardLast(event) {
    event.preventDefault();
    parentNodeCards.insertBefore(parentNodeCards.children[0], parentNodeCards.children[5].nextSibling);
}
leftButtonField.addEventListener("click", firstCardLast);
...
// Indice 1 : L'event à utiliser est "keypress", 
// appliqué au texte "JS & Events" (ou, mieux, à une div le contenant).

// Indice 2 : Pour modifier le rendu visuel de toute la page, 
// il faut que tu joues avec les classes de <body>.

// Indice 3 : Pour mettre tout sur 4 colonnes, rajoute la classe col-4. 
// Pour les placer au milieu ou à droite, rajoute la classe offset-md-4 ou offset-md-8.

// Indice 4 : Pour éviter que les classes s'accumulent, 
// et pour que chaque touche ait un comportement propre, 
